	...
	if ($table == 'site_languages') {
		...
		foreach($items as $item_id)
		{
			...
			{	
				...
				case 'th_layouts':
					$dq = "DELETE FROM osc_th_layouts_items WHERE `ref_id`= '$item_id'";
					$db->q($dq,0,1);
					break;
				case 'sys_doc_cats':
					$dq = "DELETE FROM osc_sys_docs WHERE `cat`= '$item_id'";
					$db->q($dq,0,1);
					break;
				case 'sys_flats':
					$dq = "DELETE FROM osc_sys_flats_layouts WHERE `flat_id`= '$item_id'";
					$db->q($dq,0,1);
					break;
			...
			{
				if($table == 'sys_events'){
					$event_info = $db->q("SELECT M.type, M.name FROM osc_sys_events as M WHERE `id`='$item_id' LIMIT 1");
					...
					$qev = "UPDATE `".$layout_table."` SET `event_id` = '0' WHERE `event_id` = '$item_id'";

					$db->q($qev,0,1);
				}

				if($table == 'sys_townhouses'){
					$qqv = "UPDATE `osc_sys_townhouses_layouts` SET `townhouse_id` = '0' WHERE `townhouse_id` = '$item_id'";
