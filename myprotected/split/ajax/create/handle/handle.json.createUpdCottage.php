<?php 
	...
	$cardUpd = array(
					...
					'price'			=> $_POST['price'],
					'area'			=> $_POST['area'],
					'cottage_desc'			=> $_POST['cottage_desc'],
	...
	{		
		...
		if($files_upload){
			...
			foreach($files_upload as $file_upload){
				$query = "INSERT INTO `osc_unc_cattages_layouts` (`filename`, `cottage_id`, `created`, `modified`, `block`) VALUES ('$file_upload', '$item_id', '$now', '$now', 0)";
				$ah->rs($query);
	...
	$data['message'] = "Коттедж успешно создан. ID =  $item_id";
	$data['status'] = "success";
