...
class NKZ extends BasicHelp
{
	...
	public function getCtReadyItem($id){
		$query = "SELECT M.* 
		FROM osc_nct_ready_ct as M WHERE `id`='$id' LIMIT 1";
		$resultMassive = $this->rs($query);
		...
		if ($items) {
			foreach ($items as $k => &$f) {
				$fid = $f['id'];
				$q = "SELECT * FROM `osc_nct_ready_ct_layouts` WHERE `ref` = '$fid' AND `block` = 0";
				$layouts = $this->rs($q);
				$f['layouts'] = $layouts;
			}
		}
	...
	public function getCtReadyItems($params=array(),$typeCount=false){
		$filter_and = "";
		if(isset($params['filtr']['massive'])){
			foreach($params['filtr']['massive'] as $f_name => $f_value){
				if($f_value < 0) continue;
